...
class SQLAir : public SQLAirBase {
   public:
    ...
    /**
     * Processes HTTP GET requests received via input stream 'is', and responds
     * with an appropriate HTTP response sent to output stream 'os'. The method
     * reads the request path and either processes a SQL Air query or returns a
     * file to the client based on the path. Exceptions generated by the
     * SQLAirBase::process are caught and reported.
     *
    ...
    /**
     * A helper method that scans a given CSV object for rows that match a
     * specified condition and updates the selected columns with the specified
     * values.
     *
     ...
     *
     * \param[in] values A vector of new values to update the selected columns
     * with.
     *
     ...
     */
    bool scanAndUpdateRows(CSV& csv, StrVec colNames, StrVec values,
                           const int whereColIdx, const std::string& cond,
   ...
   protected:
     ...
     */
    void loadFromURL(CSV& csv, const std::string& host, const std::string& port,
